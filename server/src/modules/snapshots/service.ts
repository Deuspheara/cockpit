    ...
    return this.database.sql.begin(
      ...
      async (tx) => {
        ...
        const eligible = dashboard.accounts.filter(
          (a) =>
            a.complete && (a.sourceType === "manual" || a.isDemo || !a.stale),
        );
        if (!eligible.length)
          return {
            captured: false,
            reason: "No accounts have a complete current valuation",
          };
        const [batch] =
        ...
        for (const a of eligible)
          await tx`INSERT INTO account_valuations(batch_id,account_id,total_value,currency) VALUES(${String(batch.id)},${a.id},${a.value},${currency})`;
