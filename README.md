...
- Shared ISIN security resolution for CSV, screenshot and manual entries; verified EODHD daily prices/history, OpenFIGI identity evidence, durable PostgreSQL refresh jobs and explicit partial valuation.
...
For listed securities, set `EODHD_API_TOKEN`. `EODHD_DAILY_LIMIT` defaults to 20 calls/day and accepts up to 100,000; `EODHD_PER_MINUTE_LIMIT` controls burst usage. `OPENFIGI_API_KEY` is optional and improves identity-mapping throughput, but OpenFIGI results are evidence only—only exact-ISIN verified EODHD mappings can price a holding. The worker resolves and refreshes asynchronously, so imports remain successful when providers are unavailable. Review ambiguous listings under Settings → Tools → Market data. See [market-data pipeline](docs/market-data.md).
...
- Valuation history combines reconstructed manual-account ledger history with connected-provider history. Linked ISIN holdings use raw EOD closes and dated ECB FX, carrying prices/FX across closures for no more than seven calendar days; legacy snapshots remain audit records. Historical USD equity converts with dated ECB USD/EUR references. Charts use last-observation UTC buckets: 1D hourly, 1W six-hourly, 3W/1M/3M daily, 1Y weekly; ALL adapts to coverage. Monotone interpolation smooths the line while keeping recorded values. Initial dYdX fetches are bounded to 400 daily and 300 hourly records; previously imported records remain stored. Missing prices/FX are shown as partial/unavailable, never as zero or a synthetic execution-price valuation.
